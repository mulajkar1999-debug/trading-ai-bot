...
def health():
    return jsonify({"status": "healthy", "telegram_configured": True}), 200

@app.route('/api/test-alert', methods=['GET'])
def test_alert():
    msg_id = send_telegram_alert("🧪 *Manual Test Alert*\nBot is fully operational!")
    if msg_id:
        return jsonify({"status": "success", "message": "Test alert sent to Telegram!", "message_id": msg_id}), 200
