...
# ============================================================
# STRATEGY SETTINGS (WIN-RATE OPTIMIZED)
# ============================================================
...
# Optimized Risk & Reward Management
BASE_RR = 1.50                 # Minimum 1:1.5 Risk to Reward
ATR_SL_MULTIPLIER = 1.80       # Expanded ATR Stop Loss to survive noise
BREAK_EVEN_TRIGGER_R = 1.00    # Move to BE only after 1.0R profit
BREAK_EVEN_BUFFER_R = 0.05     # Small buffer above entry
TRAIL_TRIGGER_R = 1.30         # Trailing activates after +1.30R
TRAIL_ATR_MULTIPLIER = 1.20    # Looser trailing distance
...
def send_telegram_alert(message, reply_markup=None):
    if not TELEGRAM_BOT_TOKEN:
        print("Telegram token missing. Set TELEGRAM_BOT_TOKEN in Render.")
        return None
    ...
    for attempt in range(4):
        try:
            ...
            if response.status_code == 429:
                print(f"Coinbase rate limit for {asset}. Retrying in {delay}s.")
                time.sleep(delay)
                delay *= 2
                continue

            if response.status_code != 200:
                print(f"Coinbase error {asset}: {response.status_code}")
                return None
            ...
            print(f"Coinbase request error {asset}: {e}")
            time.sleep(delay)
            delay *= 2
        except Exception as e:
            print(f"Coinbase parsing error {asset}: {e}")
            return None
...
def calculate_adx(highs, lows, closes, period=14):
    ...
    for i in range(period, len(tr_list)):
        ...
        dx = 100 * abs(plus_di - minus_di) / denominator if denominator != 0 else 0
        dx_values.append(dx)
...
# ============================================================
# SWING POINTS & PATTERNS
# ============================================================

def find_swing_highs(highs, lookback=4):
    points = []
    for i in range(lookback, len(highs) - lookback):
        if (
            highs[i] > max(highs[i - lookback:i])
            and highs[i] >= max(highs[i + 1:i + lookback + 1])
        ):
            points.append((i, highs[i]))
    return points


def find_swing_lows(lows, lookback=4):
    points = []
    for i in range(lookback, len(lows) - lookback):
        if (
            lows[i] < min(lows[i - lookback:i])
            and lows[i] <= min(lows[i + 1:i + lookback + 1])
        ):
...
def detect_patterns(highs, lows, closes):
    patterns = []
    swing_highs = find_swing_highs(highs, lookback=4)
    swing_lows = find_swing_lows(lows, lookback=4)
    ...
    # TRIPLE TOP / BOTTOM (SYNTAX ERROR FIXED HERE)
    if len(recent_highs) >= 3:
        values = [x[1] for x in recent_highs[-3:]]
        average = sum(values) / 3
        if (max(values) - min(values) <= average * 0.004) and closes[-1] < average:
...
def breakout_confirmation(highs, lows, closes, volumes, atr_value, direction):
    ...
    # Lookback expanded to last 30 candles (2.5 Hours)
...
def analyze_asset(asset):
    ...
    market = get_market_data(asset, limit=250)
    if not market:
        return None
    ...
    ema200_values = calculate_ema(closes, 200) # HTF Macro Trend Filter
    ...
    price = closes[-1]
    ema9, ema21, ema200 = ema9_values[-1], ema21_values[-1], ema200_values[-1]
    ...
    # PULLBACK ENTRY VS PEAK CHASE PENALTY
    ...
    if regime == "BULLISH":
        if 0 <= distance_to_ema <= 0.0015:
            ...
            buy_score -= 25  # Heavily penalize buying overextended peaks
        ...
        if 0 <= -distance_to_ema <= 0.0015:
            ...
            sell_score -= 25  # Heavily penalize selling overextended bottoms

    # RSI Momentum Filters
    if 50 <= rsi <= 65 and regime == "BULLISH":
        buy_score += 12
        buy_reasons.append("RSI momentum")

    if 35 <= rsi <= 50 and regime == "BEARISH":
        sell_score += 12
    ...
    if buy_score >= MIN_SIGNAL_SCORE and buy_confirmation and buy_score >= sell_score + MIN_DIRECTION_EDGE:
        # MACRO TREND FILTER: Do not buy below EMA200
        if price >= ema200:
            action = "BUY"
            score = buy_score
            reasons = buy_reasons

    elif sell_score >= MIN_SIGNAL_SCORE and sell_confirmation and sell_score >= buy_score + MIN_DIRECTION_EDGE:
        # MACRO TREND FILTER: Do not sell above EMA200
        if price <= ema200:
            action = "SELL"
            score = sell_score
    ...
    sl_distance = max(atr * ATR_SL_MULTIPLIER, minimum_sl)
...
def cooldown_active(asset):
    ...
    if not last:
        return False
    return (datetime.now() - last) < timedelta(minutes=SIGNAL_COOLDOWN_MINUTES)


def analyze_and_trigger(asset):
    if cooldown_active(asset) or has_active_signal(asset):
        return
...
def update_trade_management(signal, current_price, current_atr):
    entry, risk = signal["price"], signal["risk_distance"]
    if risk <= 0:
        return
    ...
    # BREAK EVEN TRIGGER
    if not signal["breakeven_done"] and current_r >= BREAK_EVEN_TRIGGER_R:
        buffer = risk * BREAK_EVEN_BUFFER_R
        if is_buy:
            new_sl = entry + buffer
            if new_sl > signal["sl"]:
    ...
    if signal["breakeven_done"] and current_r >= TRAIL_TRIGGER_R:
        trail_distance = current_atr * TRAIL_ATR_MULTIPLIER
        if is_buy:
            new_sl = signal["best_price"] - trail_distance
            if new_sl > signal["sl"]:
                signal["sl"] = round(new_sl, 2)
...
def update_history(result, r_value):
    with state_lock:
        if result == "WIN":
            ...
            trade_history["gross_loss_r"] += abs(min(r_value, 0))
        else:
        ...
        gross_profit, gross_loss = trade_history["gross_profit_r"], trade_history["gross_loss_r"]
        if gross_loss > 0:
    ...
    while True:
        try:
            for signal in list(active_signals):
                ...
                highs, lows, closes = market["highs"], market["lows"], market["closes"]
                if not closes:
                    continue

                current_price, current_high, current_low = closes[-1], highs[-1], lows[-1]
                atr_values = calculate_atr(highs, lows, closes, 14)
                ...
                is_buy = (signal["action"] == "BUY")
                tp_hit = (current_high >= signal["tp"]) if is_buy else (current_low <= signal["tp"])
                sl_hit = (current_low <= signal["sl"]) if is_buy else (current_high >= signal["sl"])
                ...
                if tp_hit and sl_hit:
                    ...
                    if signal["breakeven_done"] and r_value >= -0.10:
                        result = "BREAKEVEN"
                        status = "🛡️ BREAK-EVEN"
                    else:
                        result = "LOSS"
                ...
                message = (
                    ...
                    f"Wins: `{trade_history['wins']}` | Losses: `{trade_history['losses']}`\n"
...
def home():
    return jsonify({
        "status": "5M Scalping Bot V2 Active",
