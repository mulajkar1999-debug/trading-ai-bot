...
def get_market_klines(asset_name, interval="15m", limit=210):
    ...
    # 1. Fetch REAL FOREX XAU/USD via TwelveData API
    ...
    # 2. Fetch BTC-USD via Coinbase API
    product_id = "BTC-USD"
...
def home():
    return jsonify({"status": "TwelveData Real Forex Engine Active 🚀"}), 200
...
def get_signal():
    ...
    if not data:
        return jsonify({"status": "Connecting Live TwelveData feed... Please refresh."}), 200
